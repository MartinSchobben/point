...
eval_diag <- function(.IC, .ion1, .ion2, ..., .nest = NULL, .X = NULL,
                      ...
                      .execution = NULL, .hat_E = NULL, .output = "inference",
  ...
  args <- inject_args(
    .IC,
    enquos(.execution = .execution, .flag = .flag, .hat_E = .hat_E),
    type = c("diagnostics"),
    check = FALSE
  )  |>
    append(args)
  ...
  IC_n <- dplyr::count(.IC, !!! gr_by, !! args[[".flag"]], !! args[["ratio"]],
                       !! args[["M_R"]]) |>
    # if provided with external data mean can be calculated for e.g. blocks and
    # should be corrected for
    dplyr::group_by(!!! gr_by, !! args[[".flag"]]) |>
...
cstd_var <- function(.IC, gr_by, args){
  dplyr::group_by(.IC, !!! gr_by) |>
    dplyr::mutate(
      ...
      # weighted residuals
      !!args[[".hat_E"]] := !!args[[".hat_E"]] * (1 / weight),
